...
function WorldTransferPopup:CheckUserHasOrders(successCallback, failureCallback)
  ...
  self.searchRequestId = contractsDataHandler:LookupContractsForLocalPlayer(self, function(self, rawContracts)
    ...
    local numOutstanding = 0
    for i = #contracts, 1, -1 do
      local contractData = contracts[i]
      if contractData.statusEnum ~= eContractStatus_Resolved and contractData.statusEnum ~= eContractStatus_Completed and (contractData.contractType == eContractType_Sell or contractData.contractType == eContractType_Buy) then
        numOutstanding = numOutstanding + 1
      end
    end
    successCallback(self, numOutstanding)
  end, failureCallback, eContractEventOpenFilter_All, eContractEventRoleFilter_Creator, paginationData.contractsPerPage, paginationData)
end
