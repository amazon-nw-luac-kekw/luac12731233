...
function WorldSelectListItem:SetWorldInfo(worldInfo, isInTransferPopup)
  ...
  self:SetStatus(self.worldInfo.worldData.status, self.worldInfo.worldData.publicStatusCode, isInTransferPopup)
...
function WorldSelectListItem:SetStatus(status, publicStatus, isInTransferPopup)
  ...
  self.isTransferDisabled = bitHelpers:TestFlag(publicStatus, bitHelpers.SERVERSTATUS_NOCHARACTERTRANSFER)
  self.isServerMessageVisible = self.isDisabled or self.isServerDown or self.isAtCharacterLimit or self.isCharacterCreationDisabled or self.isTransferDisabled
  self:SetEnabled(not self.isDisabled)
  local scrimDisabledAlpha = 0
  if self.isServerMessageVisible then
    if self.isDisabled or self.isServerDown or self.isAtCharacterLimit or not isInTransferPopup and self.isCharacterCreationDisabled then
      scrimDisabledAlpha = 0.5
    elseif self.isTransferDisabled then
      if isInTransferPopup then
        scrimDisabledAlpha = 0.5
      else
        scrimDisabledAlpha = 0
      end
    end
  end
  ...
  UiTextBus.Event.SetFont(self.Properties.WorldName, self.UIStyle.FONT_FAMILY_NIMBUS_REGULAR)
  UiTextBus.Event.SetFont(self.Properties.CharacterName, self.UIStyle.FONT_FAMILY_NIMBUS_REGULAR)
  UiTextBus.Event.SetFont(self.Properties.WorldSet, self.UIStyle.FONT_FAMILY_NIMBUS_REGULAR)
  ...
  elseif self.isCharacterCreationDisabled and not isInTransferPopup then
    serverMessageColor = self.UIStyle.COLOR_YELLOW
  elseif self.isTransferDisabled then
    if isInTransferPopup then
      serverMessageColor = self.UIStyle.COLOR_YELLOW
    else
      serverMessageColor = self.UIStyle.COLOR_WHITE
    end
  end
  if self.isAtCharacterLimit then
    serverMessageColor = self.UIStyle.COLOR_GRAY_70
    characterMessageColor = self.UIStyle.COLOR_YELLOW
    self:SetTooltip("@mm_world_character_limit")
  elseif self.isServerDown then
    self:SetTooltip("@error_world_select_server_down")
  elseif self.isCharacterCreationDisabled and not isInTransferPopup then
    self:SetTooltip("@mm_characterCreateDisabled")
  elseif self.isTransferDisabled then
    if isInTransferPopup then
      self:SetTooltip("@mm_charactertransferdisabled")
    else
      self:SetTooltip("")
    end
  else
    self:SetTooltip("")
    UiTextBus.Event.SetFont(self.Properties.WorldName, self.UIStyle.FONT_FAMILY_NIMBUS_MEDIUM)
    UiTextBus.Event.SetFont(self.Properties.CharacterName, self.UIStyle.FONT_FAMILY_NIMBUS_MEDIUM)
    UiTextBus.Event.SetFont(self.Properties.WorldSet, self.UIStyle.FONT_FAMILY_NIMBUS_MEDIUM)
  end
...
function WorldSelectListItem:OnUnfocus()
  ...
  if isSelectedState == true and self.isSelectable then
    self:OnSelected(nil, true)
  else
    self:OnUnselected()
  end
end
function WorldSelectListItem:OnSelected(entityId, skipCallback)
  if self.isSelectable then
    self.ListItemBg:OnFocus()
  end
  if skipCallback ~= true and self.onSelectedCallerTable then
    self.onSelectedCallback(self.onSelectedCallerTable, self)
  end
end
function WorldSelectListItem:SetSelectedCallback(callerTable, callbackFunction)
  self.onSelectedCallerTable = callerTable
  self.onSelectedCallback = callbackFunction
end
