local CharacterCreateButton = {
  ...
  isContextualButtonsShowing = false
}
...
function CharacterCreateButton:OnInit()
  ...
  self.contextButtons = {
    self.Properties.DeleteButton,
    self.Properties.CharacterTransferButton
  }
  if self.Properties.CharacterTransferButton:IsValid() then
    self.CharacterTransferButton:SetText("@server_transfer_change_world")
    self.CharacterTransferButton:SetEnabled(false)
    self.CharacterTransferButton:SetTooltip("@server_transfer_change_world_tooltip")
  end
...
function CharacterCreateButton:SetContextualButtonsVisible(isVisible)
  self.isContextualButtonsShowing = isVisible
  if isVisible then
    self:OnChangeCharacter()
  end
  for _, button in ipairs(self.contextButtons) do
    self.ScriptedEntityTweener:Stop(button)
    if isVisible then
      UiElementBus.Event.SetIsEnabled(button, true)
      self.ScriptedEntityTweener:Play(button, 0.3, {opacity = 1, ease = "QuadOut"})
    else
      self.ScriptedEntityTweener:Play(button, 0.3, {
        opacity = 0,
        ease = "QuadOut",
        onComplete = function()
          UiElementBus.Event.SetIsEnabled(button, false)
        end
      })
    end
  end
...
function CharacterCreateButton:ToggleDeleteButton()
  self.isContextualButtonsShowing = not self.isContextualButtonsShowing
  self:SetContextualButtonsVisible(self.isContextualButtonsShowing)
end
...
function CharacterCreateButton:SetWorldMessageTooltip(worldMessage)
  ...
  local buttonPosX = hasWorldMessage and 290 or 240
  for _, button in ipairs(self.contextButtons) do
    self.ScriptedEntityTweener:Set(button, {x = buttonPosX})
  end
end
...
function CharacterCreateButton:OnUnselected()
  self:SetContextualButtonsVisible(false)
