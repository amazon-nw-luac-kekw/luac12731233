...
function HousingManagement:OnTransitionIn(fromStateName, fromLevelName, toStateName, toLevelName)
  ...
  self.territoryNameLower = string.lower(LyShineScriptBindRequestBus.Broadcast.LocalizeText(self.territoryName))
...
function HousingManagement:OnAbandonHomeButton()
  PopupWrapper:RequestPopupWithParams({
    title = "@ui_house_abandon_this_popup_title",
    message = GetLocalizedReplacementText("@ui_house_abandon_this_popup_desc", {
      territoryName = self.territoryName
    }),
    eventId = self.abandonPopupId,
    callerSelf = self,
    callback = self.OnPopupResult,
    buttonsYesNo = true,
    yesButtonText = "@ui_house_abandon_confirm_button",
    ...
    customData = {
      {
        ...
        label = "",
        value = {
          placeholderText = "@ui_house_abandon_confirm_placeholder_text",
          callerSelf = self,
          onChangeCallback = function(callerSelf, popupRef, currentText)
            local enableButton = string.lower(currentText) == self.territoryNameLower
