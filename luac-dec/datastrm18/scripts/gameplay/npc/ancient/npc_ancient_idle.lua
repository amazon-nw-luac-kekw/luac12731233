...
function NPC_Ancient_Idle:OnActivate()
...
function NPC_Ancient_Idle:OnDeactivate()
  self.meshNotificiationHandler:Disconnect()
  self:VFX_Off()
end
function NPC_Ancient_Idle:VFX_On()
  self:VFX_Off()
  ...
  ParticleManagerBus.Broadcast.SpawnParticleAtJoint(self.Properties.NPC, "Head", "cFX_npc_Ancient.Idle_Head_Guardian", Vector3(0, 0, 0), self.isOnLocalPlayer, EmitterFollow)
end
function NPC_Ancient_Idle:VFX_Off()
...
function NPC_Ancient_Idle:OnMeshCreated()
  self:VFX_On()
end
return NPC_Ancient_Idle
